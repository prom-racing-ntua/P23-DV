...
def initSmallKeypoints(modelpath):
    model = smallKeypointsBasicCNN()
    model.load_state_dict(torch.load(modelpath,map_location=torch.device('cpu')))
    return model

def initLargeKeypoints(modelpath):
    model = largeKeypointsBasicCNN()
    model.load_state_dict(torch.load(modelpath,map_location=torch.device('cpu')))
    return model
...
def runPerception(img_path, camera, yolo_path, keypoints_path, OffsetY):
    ...
    keypoints_model = initSmallKeypoints(keypoints_path)
