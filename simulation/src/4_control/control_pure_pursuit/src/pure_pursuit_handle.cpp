...
void PurePursuitHandle::subscribeToTopics() {
    ...
    centerLineSubscriber_       =
        nodeHandle_.subscribe(center_line_topic_name_, 1, &PurePursuitHandle::centerLineCallback, this);
}
...
void PurePursuitHandle::centerLineCallback(const geometry_msgs::PolygonStamped& center_line) {
    pure_pursuit_.setCenterLine(center_line.polygon);
}
