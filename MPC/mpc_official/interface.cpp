...
double a=1.46;
double b=1.62;
//X is: X  Y  phi vx vy  r F delta index
//U is: dF ddelta, dindex
//Z is: dF ddelta dindex X  Y  phi vx vy  r F delta index
...
void getF(double X[X_SIZE],double U[U_SIZE], double (&kappa)[X_SIZE])  {
        ...
        // double xdot = (l_a)*(X[3]*std::cos(X[2]) - X[4]*std::sin(X[2])) + (1-l_a)*(X[3]*std::cos(X[2] + beta));
        // double ydot = (l_a)*(X[3]*std::sin(X[2]) + X[4]*std::cos(X[2])) + (1-l_a)*(X[3]*std::sin(X[2] + beta));
        // double phidot = X[5];
        // double vxdot = (1-l_a)*((X[6] - Fdrag)/ m) + (l_a)*((X[6] - Fdrag + Ffy*std::sin(X[7]) + m*X[4]*X[5])/ m);
        // double vydot = (1-l_a)*((l_r/(l_r+l_f))*(vxdot*std::tan(X[7])+X[3]*(U[1]/std::pow(std::cos(X[7]),2)))) + (l_a)*(((-X[3]*X[5]) + (Fry + Ffy*std::cos(X[7])))/(1.0*m));
        // double rdot = (1-l_a)*((1/(l_r+l_f))*(vxdot*std::tan(X[7])+X[3]*(U[1]/std::pow(std::cos(X[7]),2)))) + (l_a)*((Ffy*l_f*std::cos(X[7]) - Fry*l_r)/(1.0*Iz));
        // double Fdot= U[0];
        // double deltadot = U[1];
        // double dindexdot = U[2];   
...
int main() {
    ...
    string filename = "data/cubic_spline_data.txt"; // replace with the name of your text file

    ifstream infile(filename);

    vector<double> X_spl, Y_spl, tang_spl, curv_spl;
