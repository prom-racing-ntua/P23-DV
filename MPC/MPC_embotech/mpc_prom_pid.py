...
class PIDController:
    def __init__(self, Kp, Kd, Ki, dt, threshold):
        self.Kp = Kp
        self.Ki = Ki
        self.Kd = Kd
        self.dt = dt
    ...
    def filter(self,value,threshold):
        if(value>threshold): return threshold
        elif(value<-threshold): return -threshold
        else: return value

    def compute(self, error):
        self.integral_error += error*self.dt
        print("self kp, error, thres and mult are ",self.Kp,error,threshold,self.Kp*error)
        P = self.filter(self.Kp*error,threshold)
        I = self.filter(self.Ki*self.integral_error,threshold)
        D = self.filter(self.Kd*((error - self.prev_error)/self.dt),threshold)
        output = self.filter(P + I + D,threshold)
...
def main():
    ...
    #PID class
    pid_controller = PIDController(Kp,Kd,Ki,dt_integration,threshold)
    ...
    for k in range(sim_length):
        ...
        # u[:,k] = pred_u[:,0]
        print("vx_target, vx_actual and error are", x[3,k], problem["all_parameters"][11][0],x[3,k]-problem["all_parameters"][11][0])
        F_pid = pid_controller.compute(x[3,k]-problem["all_parameters"][11][0])
        print("Fout are ",F_pid)
        u[:,k] = pred_u[:,2]
