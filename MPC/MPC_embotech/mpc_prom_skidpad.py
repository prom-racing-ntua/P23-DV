...
def obj(z,current_target):
    ...
    return (3e3*(z[3]-current_target[0])**2 # costs on deviating on the path in x-direction
            ...
            + 1e-3*(sar**2)
            + 1e-3*(dsa**2)
            + 5e2*((z[9]/(a*Frz))**2 + (Fry/(b*Frz))**2)
            # + 1e-1*((1/(z[6]**2 +1e-3))) #vx and index
            - 1e-5*(z[6])
            - 1e-5*(z[11]/INDEX_MAX_ALL)
            - 1e-5*(z[2]/INDEX_MAX_ALL))
...
def generate_pathplanner():
    ...
    model.ub = np.array([+3560.7*eff,  np.deg2rad(+30), INDEX_MAX_ALL, 400.,   400.,   +np.inf, 12.5, +15.0, 15.0, 3560.7*eff, np.deg2rad(30), INDEX_MAX_ALL*10])
    ...
    # -----------------
    # Set solver options
    codeoptions = forcespro.CodeOptions('FORCESNLPsolver_skidpad')
...
def cubic_spline_inference(cs,parameter,x):
    ...
    for i in range(np.shape(parameter)[0]):
        ...
        u_first=np.sqrt((b*g/(curvature+1e-3)))
...
def main():
    ...
    for k in range(sim_length):
        ...
        if(k>0):
            ...
            if(err_1 < 0.9):
                ...
                for i in range (model.N-1):
                    ...
                    s_solver=pred_x[8,i] 
                    if(s_new>INDEX_MAX): 
                        if(counter_all==0 or counter_all==5):s_new = INDEX_MAX
                ...
                ds_emerg=0.1
                if(s_start > idx_start): s_start = idx_start
                ...
                x_for_splines=problem["xinit"]
                next_data_points_em = cubic_spline_inference(cs,np.array(parameters_array_emergency),x_for_splines)
        ...
        print("ellipse array is: ",ellipse_array)
