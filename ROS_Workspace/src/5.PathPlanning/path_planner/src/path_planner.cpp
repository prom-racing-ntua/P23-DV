...
void Path_Planner_Node::mapping_callback(const custom_msgs::msg::LocalMapMsg::SharedPtr msg)
{
    ...
    Point current_direction(std::cos(theta), std::sin(theta));
    ...
    for_pub.set__count(waypoints.size());
    ...
    {
        sample.set__x(point.x());
        sample.set__y(point.y());
        waypoints_ros.push_back(sample);
    }
    for_pub.set__waypoints(waypoints_ros);
