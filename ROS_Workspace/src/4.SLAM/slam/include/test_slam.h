...
public:
	...
	UnaryFactor(gtsam::Key j, double m_x, double m_y, double x, double y, const gtsam::SharedNoiseModel& model);
...
void GraphSLAM<T>::addLandmarkMeasurementSLAM(const unsigned long global_index, std::vector<PerceptionMeasurement>& landmarks) {
	...
	for (PerceptionMeasurement& cone : landmarks)
	{
		...
		observed_position << observation_pose.x() + cone.range, observation_pose.y() + cone.theta;
		...
		new_factors_.add(gtsam::DeltaFactor(observation_pose_symbol, best_match->symbol, gtsam::Point2(cone.range, cone.theta), noise_model));
	}
}
