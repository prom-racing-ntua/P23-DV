...
def finalCoordinates(camera, classes, cropped_img_corners, predictions, OffsetY):
    ...
    for j in range(len(classes)):
        ...
        # Sometimes the KeypointNet mispredicts the coordinates of one or more keypoints
        # To prevent garbage measurements from being sent to SLAM a reprojection error check is included
        # Supposing the camera is placed where solvePnP says it is compared to the cone in question,
        # how far from the original pixels would the keypoints project in this case?
        # We use a threshold that depends directly on the size of the bounding box in the original image
