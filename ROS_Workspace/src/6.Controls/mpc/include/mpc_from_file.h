...
void Initialize_all(){
    ...
    const double xinit_temp[9] = {X_spl[0], Y_spl[0],tang_spl[0],0.0,0.0,0.0,3560/10,0.0,0.0};
...
void readData() {
    std::string filename = "src/6.Controls/mpc/data/cubic_spline_data.txt"; // replace with the name of your text file
...
void writeParams() {
    ...
    for(int k = 0; k < 4*LOOKAHEAD; ++k) { //set parameters
        ...
        if (mod_ == 0) {
            params.all_parameters[k] = X_spl[int(params.xinit[8])+ int(k/4)];
            // std::cout << "added X at kappa  " << k << std::endl;
        }
        else if(mod_ == 1) {
            params.all_parameters[k] = Y_spl[int(params.xinit[8])+ int((k-1)/4)];
            // std::cout << "added Y at kappa " << k << std::endl;
        }
        else if(mod_ == 2) {
            params.all_parameters[k] = tang_spl[int(params.xinit[8])+int((k-2)/4)];
            // std::cout << "added tang at kappa " << k << std::endl;
        }
        else {
            params.all_parameters[k] = curv_spl[int(params.xinit[8])+int((k-3)/4)];
            // std::cout << "added curv at kappa " << k << std::endl;
        }
        }
        end_params=clock();
        double time_taken_params = double(end_params - st_params) / double(CLOCKS_PER_SEC);
        std::cout << "Time for parameter estimation is: " << std::fixed << time_taken_params*1000 << std::setprecision(9); 
        std::cout << " ms " << std::endl;
}
} //end namespace