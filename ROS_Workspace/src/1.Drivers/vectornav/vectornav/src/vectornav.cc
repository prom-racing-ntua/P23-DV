...
public:
  Vectornav() : Node("vectornav") {
    ...
    RCLCPP_INFO(get_logger(), "Port param is: %s", port.c_str());
    ...
    // Antenna Offset and Compass Baseline
    declare_parameter<double>("GpsAntennaOffset.x", 0.0);
    declare_parameter<double>("GpsAntennaOffset.y", 0.0);
    declare_parameter<double>("GpsAntennaOffset.z", 0.0);
    declare_parameter<double>("GpsBaselinePos.x", 1.0);
    declare_parameter<double>("GpsBaselinePos.y", 0.0);
    declare_parameter<double>("GpsBaselinePos.z", 0.0);
    declare_parameter<double>("GpsBaselineUncertainty.x", 0.0254);
    declare_parameter<double>("GpsBaselineUncertainty.y", 0.0254);
    declare_parameter<double>("GpsBaselineUncertainty.z", 0.0254);
...
private:
  ...
  bool connect(const std::string port, const int baud) {
    ...
    // Gps Antenna Offset and Compass Baseline
    vn::sensors::GpsCompassBaselineRegister configBase;
    double offset_x = get_parameter("GpsAntennaOffset.x").as_double();
    double offset_y = get_parameter("GpsAntennaOffset.y").as_double();
    double offset_z = get_parameter("GpsAntennaOffset.z").as_double();

    vn::math::vec3f antenna_offset(offset_x, offset_y, offset_z);
    vs_.writeGpsAntennaOffset(antenna_offset);

    if (vs_.determineDeviceFamily() == vn::sensors::VnSensor::VnSensor_Family_Vn300)
    {
      configBase.position[0] = get_parameter("GpsBaselinePos.x").as_double();
      configBase.position[1] = get_parameter("GpsBaselinePos.y").as_double();
      configBase.position[2] = get_parameter("GpsBaselinePos.z").as_double();
      configBase.uncertainty[0] = get_parameter("GpsBaselineUncertainty.x").as_double();
      configBase.uncertainty[1] = get_parameter("GpsBaselineUncertainty.y").as_double();
      configBase.uncertainty[2] = get_parameter("GpsBaselineUncertainty.z").as_double();
      vs_.writeGpsCompassBaseline(configBase);
    }
