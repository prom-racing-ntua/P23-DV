...
namespace can_reader_namespace {
    ...
    void CanReader::read2byte(unsigned char *from, unsigned int *to, int n) {
        RCLCPP_INFO(get_logger(), "read2byte started");
        for (int i=0; i<n; i++) {
            unsigned int x = (charToInt(from[i*2]) << 4) + charToInt(from[i*2+1]);
            to[i] = x;
            //RCLCPP_INFO(get_logger(), "%02X", to[i]);
        }
        RCLCPP_INFO(get_logger(), "read2byte finished");
    }

    unsigned int CanReader::charToInt(unsigned char r) {
        RCLCPP_INFO(get_logger(), "charToInt started");
        int x=(int)r;
        if (r >= '0' && r <= '9'){
            return x-'0';
        }
        ...
        return 0;
        RCLCPP_INFO(get_logger(), "charToInt finished");
    }


    void CanReader::read_serial() {
        RCLCPP_INFO(get_logger(), "read serial started");
        ...
        if (ready == -1) {
            ...
            RCLCPP_INFO(get_logger(), "after read");

            read2byte(read_buf, byte_array, 11);
            RCLCPP_INFO(get_logger(), "extract_message started");
            CanMessage *can_message = extract_message(byte_array);
            RCLCPP_INFO(get_logger(), "extract_message finished");
            if (can_message != NULL) {
                RCLCPP_INFO(get_logger(), "handling message");
                can_message->handler();
                RCLCPP_INFO(get_logger(), "handling message ended");
            } 
            delete can_message;
            delete []read_buf;
        }
